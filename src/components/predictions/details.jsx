	...
	useEffect(() => {
		...
		const fetch_H2H = async () => {
			try {
				...
				const [h2hRes, teamALast5Res, teamBLast5Res] = await Promise.all([
					fetch(
						`https://api.2kw.net/api/v1/head2head?teamA_id=${teamA_Id}&teamB_id=${teamB_Id}`,
					),
					fetch(`https://api.2kw.net/api/v1/last5matches/team/${teamB_Id}`),
					fetch(`https://api.2kw.net/api/v1/last5matches/team/${teamA_Id}`),
				]);

				if (!h2hRes.ok || !teamALast5Res.ok || !teamBLast5Res.ok) {
				...
				const response = {
					h2h: h2hData.data.response,
					last5: {
						teamA: teamALast5.data.response,
						teamB: teamBLast5.data.response,
					},
				};
						...
						<div className=' shadow-sm space-y-2 max-h-screen bg-[#F9FBFF] transition-all flex justify-between items-center w-full flex-col lg:p-14 p-2'>
							...
							{/* <div className='bg-[#1A365D] rounded-t-xl mt-4'>
								<h3 className='text-white p-4 uppercase'>
									{state.awayTeam} LAST 5 MATCHES Team_B
								</h3>
...
<section className='mt-10'>
              ...
              <div className='bg-[#1A365D] rounded-t-xl'>
								<h3 className='text-white p-4'> {state.awayTeam} LAST 5 MATCHES</h3>
							</div>
							{loading ? (
								<div className=' hover:shadow-lg text-[#1a365d] py-20 hover:bg-[#FFF7E0] group transition-all flex justify-center items-center w-full rounded-[0.6rem] p-2'>
									<span>
              ...
              // ) : (
								...
								h2h.last5.teamA.map((item, index) => (
           ...
           <section className='mt-10'>
              ...
              <div className='bg-[#1A365D] rounded-t-xl'>
								<h3 className='text-white p-4'> {state.homeTeam} LAST 5 MATCHES</h3>
							</div>
							{loading ? (
								<div className=' hover:shadow-lg text-[#1a365d] py-20 hover:bg-[#FFF7E0] group transition-all flex justify-center items-center w-full rounded-[0.6rem] p-2'>
									<span>
              ...
              // ) : (
								...
								h2h.last5.teamB.map((item, index) => (
