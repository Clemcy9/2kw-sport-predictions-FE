  ...
  return (
    <main className="p-4 lg:px-5 flex flex-col  w-full">
      ...
      <div className="flex justify-between font-semibold font-sans text-2xl w-full py-4  lg:py-6">
        <h1 className="lg:font-semibold font-bold font-[Inria Sans]  text-center w-full lg:text-left">
          Edit Affiliate Links
        </h1>
        <button
          onClick={() => reset_data()}
          className="hidden lg:block bg-[#DA3737] text-sm text-white px-8 py-2 shadow-sm font-normal  rounded-xl"
        >
          Cancel
        </button>
      </div>
      ...
      <form
        onSubmit={edit_Affiliate}
        onClick={() => setDropdown(false)}
        className="py-12"
      >
        <div
          className="flex lg:flex-row flex-col justify-between items-center gap-10"
          ...
          <div className="relative w-full">
            <div className=" py-3 w-full">
              ...
              <input
                ...
                required
                autoComplete="on"
                onChange={(e) => setLink_type(e.target.value)}
                type="text"
                className="w-full focus:ring focus:ring-[#1A365D] focus:border-[#1A365D] outline-none bg-white appearance-none border  border-[#1A365D] lg:border-[#00000066] p-2 rounded-xs"
                  ...
                  >
                    {dropdown_items.map((items) => (
                      <div
                        key={items.id}
                        onClick={() => {
                          setLink_type(items.title);
                          setDropdown(false);
                        }}
                        className="p-2  w-full flex justify-center items-center hover:bg-[#1A365D] text-[#1A365D] hover:text-white "
                      >
                        {items.title}
                      </div>
                    ))}
        ...
        <div className="flex flex-col py-3">
          ...
          <input
            ...
            type="text"
            className="border border-[#1A365D] lg:border-[#00000066] p-2 rounded-xs"
          ...
          <aside className="flex items-center gap-5">
            <div className="flex justify-center items-center gap-1 text-[#00000066] font-semibold">
              ...
              <input
                ...
                name="visible"
                id="visible"
                className=" accent-[#1A365D] w-5 h-5 rounded-[0.2rem]"
              />
            </div>
            <div className="flex justify-center items-center gap-1 text-[#00000066] font-semibold">
              ...
              <input
                ...
                name="visible"
                id="visible"
                className="accent-[#DA3737] w-5 h-5 rounded-[0.2rem]"
              />
            </div>
          </aside>
        </div>
        ...
        <div className="flex justify-between items-center">
          <button
            type="submit"
            className="w-40 lg:w-auto bg-[#1A365D] text-sm text-white lg:px-10 lg:py-2 py-3 shadow-sm font-normal  rounded-xl"
          >
            Save Content
          </button>
          <button
            ...
            className="w-40 lg:hidden bg-[#DA3737] text-sm text-white lg:px-8 lg:py-1.5 py-3 shadow-sm font-normal  rounded-xl"
          >
            Cancel
          </button>
        </div>
        {error && <h2 className="text-red-700 p-4">{error}</h2>}
      </form>
      {modal && <AnimationModal title="Link Created Successfully" />}
    </main>
  );
}
