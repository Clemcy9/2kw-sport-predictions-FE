...
export default function All_Legues() {
    ...
    <div className="hidden p-4 lg:px-5 lg:min-h-screen  flex-col w-full">
      ...
      <div className="w-full flex flex-col lg:flex-row lg:justify-between lg:my-4  gap-7 lg:gap-5">
        <div className="relative lg:w-60 w-full flex justify-between items-center">
          <input
            value={byName}
            onKeyDown={(e) => e.key === "Enter key" && all_predictions}
      ...
      <table className=" w-full border-collapse">
        <thead className="">
          <tr className="font-bold text-left text-lg hidden lg:grid grid-cols-12 justify-between gap-6  w-full">
            <div className="col-span-8 justify-center grid grid-cols-12">
              <th className="py-3 col-span-3">No.</th>
              <th className="py-3 col-span-3">League ID</th>
              <th className="py-3 col-span-3">Country</th>
              <th className="py-3 col-span-3">Name</th>
            </div>
            <div className="col-span-4 flex justify-between w-full">
              <th className="py-3">Short-Name</th>
            ...
            <tr key={item.fixture.id} className="leading-tight">
              <div className="w-full flex flex-row lg:gap-2 lg:grid lg:grid-cols-12 lg:justify-between lg:border-none border p-3 lg:p-0 rounded-xl my-4 lg:my-0 lg:active:hidden active:border-[#1A365D] active:scale-105 active:shadow-xl border-[#1A365D99]">
                <div className="flex flex-col lg:col-span-8 lg:grid lg:grid-cols-9 justify-between  items-start gap-6 lg:flex-row w-full lg:gap-1 ">
                  <td className="lg:py-5 hidden lg:col-span-1 lg:block lg:mr-12">
                    {index + 1}
                  </td>
                  <td className="lg:py-5  lg: flex lg:justify-start lg:col-span-4 lg:items-start w-full lg:max-w-80 ">
                    <img
                      className="w-8 h-8"
                      src={item.league.logo}
                      alt={item.league.name}
                    />
                    {(item?.league?.name || "").slice(0, 32).trim()}
                  </td>
                  <td className="lg:py-5 lg:flex lg:items-start lg:justify-start lg:col-span-4 lg:w-full lg:max-w-72 lg:flex-row">
                    <span className="flex">
                      <img
                        className="w-4 h-4"
                      ...
                      />{" "}
                      {(item?.teams?.away.name || "").slice(0, 16).trim()}
                    </span>{" "}
                    ...
                    <span className="flex">
                      ...
                      />{" "}
                      {(item.teams.home.name || "").slice(0, 16).trim()}
                    </span>
                  </td>
                </div>
                <div className="flex gap-8 font-light lg:col-span-4 font-sans lg:text-lg lg:font-normal text-xs text-[#737373] items-end lg:flex-row flex-col lg:w-auto lg:justify-center lg:gap-18">
                  <td className="lg:py-5  ">
                    {new Date(item.fixture.date).toLocaleDateString()}
                  </td>
                  <td className="lg:py-5 pr-2 w-20 lg:pr-0 ">
                  ...
                  <td className="lg:py-5 lg:ml-4 lg:pr-0 pr-10">
                    <button className="text-[#04BA4A] transition">
