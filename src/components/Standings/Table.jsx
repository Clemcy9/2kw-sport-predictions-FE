...
export default function LeagueTables({ league }) {
  ...
  if (error)
    return (
      <div className="text-center flex justify-center items-center text-red-500 py-20  w-full rounded-xl">
        <FaTools /> {error}
      </div>
    );
          ...
          <div className="flex lg:justify-start gap-2 bg-[#E5E7EB]/80 w-full rounded-[0.4rem] p-1">
            {leagues.map((lg) => (
              <button
                ...
                className={`px-3 py-2 text-lg rounded-[0.4em] transition-all ${
                  activeLeague === lg
                ...
                <table className="w-full text-lg text-left">
                  ...
                  <tbody className="p-2 text-sm">
                    {leagueData.league.map((item, i) => (
                      ...
                      >
                        <td className="py-1 px-1 text-[#1A365D]">
                          {item.rank}
                        </td>
                        ...
                        <td className="py-1 px-1 font-semibold text-[#1A365D]">
                          {item.pts}
                        </td>
                        <td className="py-1 px-1 text-[#1A365D]/90">
                          {item.w}
                        </td>
                        <td className="py-1 px-1 text-[#1A365D]/90">
                          {item.d}
                        ...
                        <td className="py-1 px-1 text-green-600 font-medium">
                          {item.gd}
                        </td>
                      </tr>
                    ))}
                      ...
                      >
                        <td className="flex items-center gap-2 py-1 px-1">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-[#1A365D]/80">
                              {item.team}
                            </div>
                          </div>
                        </td>
                        <td className="py-1 px-1 text-[#1A365D]/80">
                          {item.matches}
                        </td>
                        <td className="py-1 px-1 font-semibold text-[#D6AE3E]">
                          {item.goals}
                        </td>
                      </tr>
                    ))}
