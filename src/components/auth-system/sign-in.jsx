...
export default function SignIn() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.msg || "sign-In not successful");
      setLoading(false);
    ...
    <div>
      <div className="overflow-y-hidden md:min-h-screen flex justify-center md:items-center bg-white w-full">
        <div className="w-full md:w-auto md:bg-[#C1CAD8] md:shadow-lg px-4 py-4 space-y-5 flex md:justify-center md:items-center flex-col md:px-20 md:mx-0 mx-2">
          <h1 className="text-3xl cursor-pointer flex items-center font-bold justify-center font-serif text-[#1A365D] pt-5">
            <motion.span
              animate={{ rotate: 360 }}
          ...
          <form
            ...
            className="w-full md:w-auto justify-center items-center flex-col flex space-y-4 md:space-y-0 "
          >
            ...
            <div className="flex flex-col items-start w-full sp">
              <Link to={"/forgot-password"} className="text-[#1A365D] ">
                Forgot Password
              </Link>
            ...
            <button
              type="submit"
              className="bg-[#1A365D] flex justify-center items-center text-white w-70 rounded-[0.6rem] py-2 mt-6"
            >
              {loading ? (
