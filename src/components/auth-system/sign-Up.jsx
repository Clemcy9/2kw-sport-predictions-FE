...
export default function SignUp() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch("https://api.2kw.net:5000/api/v1/auth/register", {
        method: "POST",
  ...
  return (
    <div className="flex justify-center md:items-center md:min-h-screen bg-white w-full overflow-y-hidden">
      <div className=" w-full md:w-auto md:bg-[#C1CAD8] md:shadow-lg px-4 py-4 space-y-5 flex md:justify-center md:items-center flex-col md:px-20 md:mx-0 mx-2">
        <h1 className="text-3xl cursor-pointer flex items-center font-bold justify-center font-serif text-[#1A365D] pt-3">
          <motion.span
            animate={{ rotate: 360 }}
        ...
        <form
          ...
          className="w-full md:w-auto justify-center items-center flex-col flex md:space-y-4 "
        >
          ...
          <div className="w-full my-3 ">
            <label className="block text-[#1E1E1E] font-sans">
              Confirm Password
            </label>
            <div className="relative">
              <input
          ...
          <button
            disabled={loading}
            type="submit"
            className="flex justify-center items-center bg-[#1A365D] text-white w-70 rounded-[0.6rem] py-2 mt-4"
          >
            {loading ? (
